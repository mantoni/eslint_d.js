...
describe('lib/forwarder', () => {
  ...
  context('isAlive', () => {
    ...
    it('handles correct response', async () => {
      const chunks = ['{"isAlive":true}'];
      sinon.replace(
        socket,
        'read',
        sinon.fake(() => (chunks.length ? chunks.shift() : null))
  ...
  context('forwardToDaemon', () => {
    ...
    it('writes token, color level, cwd and argv to socket', () => {
      ...
      assert.calledOnceWith(
        socket.write,
        `["token",null,${color_level},"the/cwd",[${JSON.stringify(process.argv0)},"eslint_d"]]`
      );
    ...
    context('--fix-to-stdout', () => {
      ...
      it('replaces the option with --fix-dry-run --format json', async () => {
        ...
        assert.calledWith(
          socket.write,
          `["token",null,${color_level},"cwd",[${JSON.stringify(process.argv0)},"eslint_d","--stdin","--fix-dry-run","--format","json","--other","--options"]]`
        );
        assert.calledWith(socket.write, '\n');
        assert.calledWith(socket.write, 'text from stdin');
        assert.calledOnce(socket.end);
