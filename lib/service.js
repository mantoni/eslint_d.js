 ...
 */
function importChalk(require) {
  // If chalk is ESM (>= v5) return chalk.default.
  // If chalk is CJS (< v5) return chalk.
  // If chalk doesn't exist as it's not required or used by eslint >=v10 return undefined.
  let chalk;
  try {
    chalk = (() => {
...
export const _test = { importChalk };
...
export function createService(resolver, token, shutdown) {
  ...
  const chalk = _test.importChalk(require);
  ...
  return (con) => {
    ...
    con
      ...
      .on('end', async () => {
        ...
        if (chalk) chalk.level = color_level;
        process.chdir(cwd);
