...
exports.lint = function (cwd, args, text) {
  ...
  const tooManyWarnings = currentOptions.maxWarnings >= 0
    && report.warningCount > currentOptions.maxWarnings;
  if (!report.errorCount && tooManyWarnings) {
    const max = currentOptions.maxWarnings;
    output += `ESLint found too many warnings (maximum: ${max}).\n`;
  }
