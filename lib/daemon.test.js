...
const WAITING_FOR_CLOSE_CONNECTIONS_DELAY = 300;
...
describe('lib/daemon', () => {
  ...
  context('ppid', () => {
    ...
    it('runs shutdown if process.kill throws', async () => {
      ...
      clock.tick(1000 + WAITING_FOR_CLOSE_CONNECTIONS_DELAY);
      await new Promise(setImmediate);
  ...
  context('idle', () => {
    ...
    it('runs shutdown after configured minutes', async () => {
      ...
      await load();
      clock.tick(2 * 60 * 1000 + WAITING_FOR_CLOSE_CONNECTIONS_DELAY);
      await new Promise(setImmediate);
    ...
    it('does not run shutdown if service is invoked', async () => {
      ...
      clock.tick(WAITING_FOR_CLOSE_CONNECTIONS_DELAY);
      await new Promise(setImmediate);
